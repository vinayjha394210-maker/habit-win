...
class _HabitHistoryScreenState extends State<HabitHistoryScreen> {
        ...
        Scaffold(
          ...
          body: Consumer<HabitService>(
            builder: (context, habitService, child) {
              ...
              final int currentStreak = habitService.calculateHabitStreak(
                currentHabit.id,
              );
              final int totalCompletionCount = habitService
                  .calculateHabitTotalCompletionCount(
                    currentHabit.id,
                  ); // New metric
              final String overallCompletionRate =
                  '${habitService.calculateOverallCompletionRateForHabit(currentHabit.id).round()}%';

              // Function to show detailed daily report for the specific habit
              void showDailyReportDialog(DateTime day) {
                final normalizedDay = date_utils.DateUtils.normalizeDateTime(
                  day,
                );
                final bool isDue = currentHabit.isHabitDueOnDate(normalizedDay);
                final bool isCompleted = currentHabit.isCompletedOn(
                  normalizedDay,
                );
                  ...
                  children: [
                    Padding(
                      ...
                      child: SizedBox(
                        ...
                        child: AbsorbPointer(
                          // Absorb pointer events to prevent internal calendar scrolling
                          child: TableCalendar(
                            ...
                            headerStyle: HeaderStyle(
                              ...
                              leftChevronIcon: CustomIcon.material(
                                Icons.chevron_left,
                              ).toWidget(defaultColor: colorScheme.onSurface),
                              rightChevronIcon: CustomIcon.material(
                                Icons.chevron_right,
                              ).toWidget(defaultColor: colorScheme.onSurface),
                            ...
                            calendarBuilders: CalendarBuilders(
                              defaultBuilder: (context, day, focusedDay) {
                                final dayStatus = _getDayCompletionStatus(
                                  day,
                                  currentHabit,
                                );
                                ...
                                if (dayStatus ==
                                    date_utils.DayCompletionStatus.perfect) {
                                  textColor = colorScheme.primary;
                                } else if (dayStatus ==
                                        date_utils.DayCompletionStatus.none &&
                                    currentHabit.isHabitDueOnDate(day) &&
                                    day.isBefore(
                                      date_utils.DateUtils.normalizeDateTime(
                                        DateTime.now(),
                                      ),
                                    )) {
                                  textColor = colorScheme.error;
                                } else if (!currentHabit.isHabitDueOnDate(
                                  day,
                                )) {
                                ...
                                if (dayStatus ==
                                        date_utils
                                            ...
                                            .perfect &&
                                    day.isBefore(
                                      date_utils.DateUtils.startOfDay.add(
                              ...
                              todayBuilder: (context, day, focusedDay) {
                                final status = _getDayCompletionStatus(
                                  day,
                                  currentHabit,
                                );
                                Color textColor = colorScheme.onSurface;
                                ...
                                if (isSameDay(day, _selectedDay)) {
                                  ...
                                  backgroundColor = colorScheme
                                      .primaryContainer; // Softer background for today
                                }
                                ...
                                if ((status ==
                                        ...
                                        (status ==
                                                date_utils
                                                    .DayCompletionStatus
                                                    .none &&
                                            currentHabit.isHabitDueOnDate(
                                              day,
                                            ) &&
                                            day.isBefore(
                                              date_utils
                                                  .DateUtils.normalizeDateTime(
                                                DateTime.now(),
                                              ),
                                            ))) &&
                    ...
                    Wrap(
                      ...
                      children: [
                        ElevatedButton(
                          onPressed: () {
                            setState(() {
                              _focusedDay = date_utils
                                  .DateUtils.normalizeDateTime(DateTime.now());
                        ...
                        SegmentedButton<CalendarFormat>(
                          ...
                          onSelectionChanged:
                              (Set<CalendarFormat> newSelection) {
                                setState(() {
                      ...
                      child: GridView.count(
                        ...
                        children: [
                          HistoryCardWidget(
                            ...
                            icon: CustomIcon.material(
                              Icons.local_fire_department,
                            ), // Updated to CustomIcon.material
                          ),
                          HistoryCardWidget(
                            title: 'HABIT FINISH',
                    ...
                    Column(
                      children: [
                        ...
                        ListView.builder(
                          shrinkWrap: true,
                          physics: const NeverScrollableScrollPhysics(),
                          itemCount: _getHabitsDueOnDay(
                            _selectedDay,
                            habitService.habits,
                          ).length,
                          itemBuilder: (context, index) {
                            final habit = _getHabitsDueOnDay(
                              _selectedDay,
                              habitService.habits,
                            )[index];
                            final isCompleted = habit.isCompletedOn(
                              _selectedDay,
                            );
                            return Card(
  ...
  date_utils.DayCompletionStatus _getDayCompletionStatus(
    DateTime day,
    Habit habit,
  ) {
    ...
    } else if (habit.isHabitDueOnDate(normalizedDay) &&
        normalizedDay.isBefore(
          date_utils.DateUtils.normalizeDateTime(DateTime.now()),
        )) {
      return date_utils.DayCompletionStatus.none;
    }
    return date_utils
        .DayCompletionStatus
        .none; // Default or other status if needed
  }

  Widget _buildHabitMarker(
    BuildContext context,
    DateTime date,
    date_utils.DayCompletionStatus status,
  ) {
    final colorScheme = Theme.of(context).colorScheme;
    Color markerColor;
